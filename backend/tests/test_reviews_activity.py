...
def test_weekly_review_upsert(client, user):
    fresh = client.get("/api/reviews/weekly", headers=user["headers"]).json()
    ...
    first = client.put("/api/reviews/weekly",
    ...
    again = client.put("/api/reviews/weekly",
                       json={"weekStart": week_start,
    ...
    got = client.get("/api/reviews/weekly", headers=user["headers"]).json()
...
def test_weekly_review_past_week_and_future_rejection(client, user):
    ...
    past = client.put("/api/reviews/weekly",
    ...
    got = client.get(f"/api/reviews/weekly?weekStart={past_anchor}",
                     headers=user["headers"]).json()
    assert got["weekStart"] == past_start
    assert got["wentWell"] == "Old week"

    future = client.put("/api/reviews/weekly",
...
def test_weekly_review_stats(client, user):
    ...
    body = client.get("/api/reviews/weekly", headers=user["headers"]).json()
    stats = body["stats"]
...
def test_reviews_isolated_between_users(client):
    _, a = register(client)
    _, b = register(client)
    week_a = client.get("/api/reviews/weekly", headers=a).json()["weekStart"]
    client.put("/api/reviews/weekly",
               json={"weekStart": week_a, "wentWell": "Secret notes"},
               headers=a)
    other = client.get("/api/reviews/weekly", headers=b).json()
...
def test_activity_feed_and_cursor_pagination(client, user):
    for i in range(3):
        ...
        client.post(f"/api/habits/{h['id']}/complete", headers=user["headers"])
    client.post("/api/goals", json={"title": "G", "category": "mind"},
                headers=user["headers"])

    full = client.get("/api/activity?limit=100", headers=user["headers"]).json()
    ...
    for _ in range(20):  # generous guard against runaway loops
        url = "/api/activity?limit=2" + (f"&cursor={cursor}" if cursor else "")
...
def test_activity_rejects_bad_inputs(client, user):
    assert client.get("/api/activity?limit=0",
                      headers=user["headers"]).status_code == 422
    assert client.get("/api/activity?limit=101",
                      headers=user["headers"]).status_code == 422
    bad = client.get("/api/activity?cursor=!!!not-base64!!!",
                     headers=user["headers"])
