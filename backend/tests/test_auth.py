...
def test_me_requires_valid_token(client):
    ...
    r = client.get("/api/users/me", headers={"Authorization": "***"})
    assert r.status_code == 401
    assert r.json()["error"]["code"] == "TOKEN_INVALID"


def test_refresh_rotates_and_revokes_old(client, user):
    r = client.post("/api/auth/refresh", json={"refreshToken": user["refresh"]})
    ...
    replay = client.post("/api/auth/refresh", json={"refreshToken": user["refresh"]})
    assert replay.status_code == 401


def test_logout_revokes(client, user):
    # Present a refresh token: it is revoked, the access token still works.
    r = client.post("/api/auth/logout", json={"refreshToken": user["refresh"]},
...
def test_update_profile(client, user):
    ...
    bad = client.patch("/api/users/me", json={"timezone": "Mars/Olympus"},
                       headers=user["headers"])
    assert bad.status_code == 422
    assert bad.json()["error"]["code"] == "VALIDATION_ERROR"

    ok = client.patch("/api/users/me", json={"timezone": "Europe/Berlin"},
                      headers=user["headers"])
    assert ok.status_code == 200 and ok.json()["timezone"] == "Europe/Berlin"


def test_change_password(client, user):
    wrong = client.post("/api/users/me/password",
                        json={"currentPassword": "nope", "newPassword": "brand-new-pw"},
                        headers=user["headers"])
    assert wrong.status_code == 401

    ok = client.post("/api/users/me/password",
