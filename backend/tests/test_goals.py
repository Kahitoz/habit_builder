...
def test_goal_crud_and_milestones(client, user):
    ...
    done = client.post(f"/api/milestones/{mid}/complete", headers=user["headers"])
    ...
    undone = client.patch(f"/api/milestones/{mid}", json={"completed": False},
                          headers=user["headers"])
    assert undone.json()["completed"] is False
    assert total_xp(client, user["headers"]) == base

    renamed = client.patch(f"/api/milestones/{mid}", json={"title": "Run 5k race"},
                           headers=user["headers"])
    assert renamed.json()["title"] == "Run 5k race"

    extra = client.post(f"/api/goals/{g['id']}/milestones",
                        json={"title": "Run a half"}, headers=user["headers"])
    assert extra.status_code == 201
    assert extra.json()["sortOrder"] == 2

    assert client.delete(f"/api/milestones/{extra.json()['id']}",
                         headers=user["headers"]).status_code == 204
    assert len(client.get(f"/api/goals/{g['id']}",
                          headers=user["headers"]).json()["milestones"]) == 2

    paused = client.patch(f"/api/goals/{g['id']}", json={"status": "paused"},
                          headers=user["headers"])
    assert paused.json()["status"] == "paused"

    assert client.delete(f"/api/goals/{g['id']}",
                         headers=user["headers"]).status_code == 204
    assert client.get(f"/api/goals/{g['id']}",
                      headers=user["headers"]).status_code == 404


def test_goal_completion_xp_round_trip(client, user):
    g = make_goal(client, user["headers"])
    client.post(f"/api/goals/{g['id']}/milestones", json={"title": "Step 1"},
                headers=user["headers"])
    base = total_xp(client, user["headers"])

    complete = client.post(f"/api/goals/{g['id']}/complete", headers=user["headers"])
    assert complete.status_code == 200
    assert complete.json()["status"] == "completed"
    assert total_xp(client, user["headers"]) == base + 1000

    # Reopening the goal reverses the goal XP (milestone XP untouched).
    reopen = client.patch(f"/api/goals/{g['id']}", json={"status": "active"},
                          headers=user["headers"])
    assert reopen.json()["status"] == "active"
    assert total_xp(client, user["headers"]) == base

    # Completing both goal and milestone, then deleting the goal, nets zero.
    mid = client.get(f"/api/goals/{g['id']}", headers=user["headers"]).json()["milestones"][0]["id"]
    client.post(f"/api/milestones/{mid}/complete", headers=user["headers"])
    client.post(f"/api/goals/{g['id']}/complete", headers=user["headers"])
    assert total_xp(client, user["headers"]) == base + 1200

    client.delete(f"/api/goals/{g['id']}", headers=user["headers"])
    assert total_xp(client, user["headers"]) == base
...
def test_goal_list_status_filter(client, user):
    ...
    client.post(f"/api/goals/{g1['id']}/complete", headers=user["headers"])

    active = client.get("/api/goals?status=active", headers=user["headers"]).json()
    completed = client.get("/api/goals?status=completed", headers=user["headers"]).json()
...
def test_habit_goal_link_and_disassociation(client, user):
    ...
    goal = client.get(f"/api/goals/{g['id']}", headers=user["headers"]).json()
    assert goal["habitCount"] == 1

    client.delete(f"/api/goals/{g['id']}", headers=user["headers"])
    after = client.get(f"/api/habits/{h['id']}", headers=user["headers"]).json()
    assert after["goalId"] is None

    bad = client.patch(f"/api/habits/{h['id']}",
...
def test_goals_isolated_between_users(client):
    ...
    assert client.get(f"/api/goals/{ga['id']}", headers=b).status_code == 404
    patch = client.patch(f"/api/goals/{ga['id']}", json={"title": "hijacked"},
                         headers=b)
    assert patch.status_code == 404
    mids = client.post(f"/api/goals/{ga['id']}/milestones",
                       json={"title": "sneaky"}, headers=b)
    assert mids.status_code == 404
    mid = ga["milestones"][0]["id"] if ga["milestones"] else None
    assert mid is None
    assert client.get("/api/goals", headers=b).json() == []
    assert len(client.get("/api/goals", headers=a).json()) == 1
...
def test_milestone_ownership_enforced(client):
    ...
    assert client.patch(f"/api/milestones/{mid}", json={"completed": True},
                        headers=b).status_code == 404
    assert client.post(f"/api/milestones/{mid}/complete",
                       headers=b).status_code == 404
    assert client.delete(f"/api/milestones/{mid}", headers=b).status_code == 404
